...
public class HabitoSistemaController {
    ...
    @PostMapping("/info/sumarpuntos")
    public ResponseEntity<Object> agregarPuntosyHabitoInfo(@RequestBody Map<String, String> requestMap) {
        try {
            String nombreUsuario = requestMap.get("nombreUsuario");
            String nombreHabito = requestMap.get("nombreHabito");

            Usuario usuario = habitoSistemaService.agregarPuntosyHabitoInfo(nombreUsuario, nombreHabito);
            return new ResponseEntity<>(usuario, HttpStatus.OK);
        } catch (IllegalStateException e) {
            String errorMessage = e.getMessage();
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorMessage);
